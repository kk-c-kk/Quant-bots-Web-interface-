...
Drop this file next to your bot (or pip-install nothing — it's stdlib only)
and report events to the central web service:

    from reporter import Reporter

    rep = Reporter(
        bot_id="sol-meanrev-v1",
        name="SOL Mean Reversion",
        asset="SOL/USDC",
        base_url="http://127.0.0.1:8000",   # service on the same VPS
        api_key=os.environ.get("REPORTER_API_KEY"),
    )

    rep.price(141.23)                                   # call every tick
    rep.trade(side="BUY", price=141.20, size=2, pnl=0)  # on fill
    rep.trade(side="SELL", price=142.10, size=2, pnl=1.80)
    rep.skip("spread too wide")                         # on skipped signal
    rep.heartbeat()                                     # if idle, every ~10s

All calls are non-blocking: events go onto an in-memory queue and a daemon
thread POSTs them. If the web service is down your bot keeps trading —
failures are LOGGED (never silently swallowed) and the event is dropped.
"""
